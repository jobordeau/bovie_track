...
namespace Tests_Fonctionnels
{
    class Program
    {
        public static Manager manager = new Manager(new Stub());
        ...
        static void Main(string[] args)
        {
            ...
            manager.AjouterFilm("La Reine des Neiges", "reine_des_neiges.jpg", new DateTime(2017, 6, 5), null, 1, new TimeSpan(1, 9, 54), new TimeSpan(0, 9, 45), 4.3f, "Vraiment pas ouf"); ;
            manager.AjouterSerie("Prison Break", "prison_break.jpg", new DateTime(2014, 8, 22), new DateTime(2020, 9, 3), 2, 5, null, null, new float?[] { 7.4f, 2.1f, null, null, null }, new string[] { "Pas mal", "Bof", null, null, null });
            manager.AjouterLivre("Les Misérables", "miserables.jpg", new DateTime(2014, 8, 12), new DateTime(2020, 4, 8), 0, 300, "Victor Hugo", null, 8.5f, null);
            Console.WriteLine("\n\n\n+++Ajouter des oeuvres+++");
            Console.WriteLine(manager);

            manager.SupprimerOeuvre(manager.GetOeuvre(2));
            Console.WriteLine("\n\n\n+++Supprimé des oeuvres+++");
            Console.WriteLine(manager);

            manager.CollectionMgr.Mode = ModeDeTri.Alphabetique;
            manager.ActualiseTrie();
            Console.WriteLine("\n\n\n+++Trie Alphabétique des oeuvres+++");
            Console.WriteLine(manager);

            manager.AjouterNouveauTag("New 1");
            manager.AjouterNouveauTag("New 2");
            manager.AjouterNouveauTag("New 3");
            manager.AjouterNouveauTag("Intéressant");
            manager.AjouterNouveauTag("test");
            Console.WriteLine("\n\n\n+++Ajouter Des tags+++");
            Console.WriteLine(manager);

            manager.AssocierTag(manager.GetOeuvre(1), "New 2");
            manager.AssocierTag(manager.GetOeuvre(1), "Intéressant");
            manager.AssocierTag(manager.GetOeuvre(1), "test");
            Console.WriteLine("\n\n\n+++Associé des tags+++");
            Console.WriteLine(manager);

            manager.EnleverTag(manager.GetOeuvre(1), "New 2");
            Console.WriteLine("\n\n\n+++Dissocié un tag+++");
            Console.WriteLine(manager);

            manager.SupprimerUnTag("test");
            Console.WriteLine("\n\n\n+++Supprimer un tag+++");
            Console.WriteLine(manager);

            int i = manager.CompterAssociationsTag("Intéressant");
            Console.WriteLine("\n\n\n+++Nombre de Tags associés+++");
            Console.WriteLine("Il y a " + i + " tags");
